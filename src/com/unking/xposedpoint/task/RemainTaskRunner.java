...
public class RemainTaskRunner extends Thread {

	public RemainTaskRunner(Context context1) {
		running = false;
		context = context1;
	}
	...
	private void runAll() {
		try {
			...
			while (localIterator.hasNext()) {
				...
				DeviceManager.getInstance().loadLastRemain(context, localApk);
				long l1;
				long l2;
				long l3;
				if (this.running) {
					/*if (DeviceManager.getInstance().getTotalRunCount(
							...
							|| Integer.parseInt(localApk.getMaxDownload()) <= 0) {*/
						while (true) {
							try {
						...
						DeviceManager.getInstance().randomLastRemain(localApk);
